...
function FiltersBar({ filters, setFilters, records, currentStages, mode = "cohort" }: { filters: Filters; setFilters: React.Dispatch<React.SetStateAction<Filters>>; records: DashboardRecord[]; currentStages?: CurrentStageRecord[]; mode?: "cohort" | "current" }) {
  ...
  if (mode === "current") {
    const currentActiveCount = [filters.managers.length > 0, Boolean(filters.pipeline), Boolean(filters.stage)].filter(Boolean).length;
...
export default function DashboardClient() {
  ...
  return <div className="app-shell">
    <aside className={menuOpen ? "open" : ""}>
      ...
      <nav>{navItems.map((item) => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => { if (item.id === "stages") setFilters((current) => ({ ...current, sources: [], period: "", sla: "", processing: "" })); changeView(item.id); setMenuOpen(false); }}><item.icon size={18} /><span>{item.label}</span>{item.id === "diagnostics" && sync.permissions.stageHistory === "error" && <i />}</button>)}</nav>
