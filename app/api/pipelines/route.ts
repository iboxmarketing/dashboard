...
export async function GET() {
  try {
    const [settings, pipelines, fields, statuses] = await Promise.all([
      getSettings(), listPipelines(), listCrmFields().catch(() => getDictionary<CrmFieldOption[]>("crmFields", [])),
    ...
    return Response.json({ pipelines, fields, stages, selectedIds: selected.map((item) => item.id), reportingIds: reporting.map((item) => item.id) });
  } catch (error) {
