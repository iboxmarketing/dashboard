...
/**
 * Historical stage-funnel rows. Same seller rule as the rest of history; the
 * projection carries no source/stage/SLA columns, so those dimensions do not
 * apply here and the view clears them when it opens.
 */
export function matchesStageHistoryFilters(
  row: { dealId: string; title?: string; salesManagerId?: string | null; originPipeline?: string },
  filters: SalesFilterSelection,
) {
  if (!matchesSelection(filters.managers ?? [], historicalManagerKey(row))) return false;
  if (filters.pipeline && row.originPipeline !== filters.pipeline) return false;
...
export function filterStageHistoryRecords<T extends { dealId: string; title?: string; salesManagerId?: string | null; originPipeline?: string }>(
  rows: readonly T[],
  filters: SalesFilterSelection,
) {
