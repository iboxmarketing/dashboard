...
test("the client wires both dimensions to the shared multi-select semantics", () => {
  ...
  assert.equal((client.match(/<MultiSelect/g) ?? []).length, 3, "manager in both modes, plus source");
  ...
  // Opening Stage Control clears the dimensions its projection cannot filter on.
  assert.match(client, /sources: \[\], period: "", sla: "", processing: ""/);
